...
class SupplierController {
    ...
    async createSupplier(req, res) {
        try {
            ...
            const existingEmail = await this.db.get('SELECT id FROM suppliers WHERE email = ?', [email]);
            if (existingEmail) {
            ...
            if (cnpj) {
                const existingCnpj = await this.db.get('SELECT id FROM suppliers WHERE cnpj = ?', [cnpj]);
                if (existingCnpj) {
            ...
            const newSupplierId = result.recordset[0].id;
