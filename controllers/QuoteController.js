...
class QuoteController {
        ...
        try {
            ...
            const quotes = await this.db.execute(`
                SELECT q.*, 
                       ...
                       (SELECT SUM(quantity * ISNULL(unit_price, 0)) FROM quote_items WHERE quote_id = q.id) as total_value
                FROM quotes q 
    ...
    static async getAll(db) {
        const sql = `
            SELECT q.*, s.name as supplier_name, s.contact as supplier_contact,
